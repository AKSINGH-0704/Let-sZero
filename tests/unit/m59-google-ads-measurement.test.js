// M59 — Google Ads measurement: consent, tag loading, and the conversion
// taxonomy.
//
...
// identical from inside the application — the page renders, nothing throws, and
...
//
// ── HARNESS NOTE ─────────────────────────────────────────────────────────────
// This repository's vitest environment is "node" — there is no jsdom and none is
 ...
 * import — hence resetModules plus a dynamic import rather than a static one.
 */
...
describe("consent authority", () => {
  ...
  it("persists a grant and reports every category", async () => {
    const { consent } = await loadModules();
    consent.acceptAll();

    expect(consent.needsConsentDecision()).toBe(false);
    expect(consent.getConsent()).toEqual({ analytics: true, advertising: true });
  });
  ...
  it("fails closed on a corrupt, truthy-but-invalid, or version-mismatched record", async () => {
    ...
    // it would grant advertising here — this is the assertion that catches it.
  });
...
describe("Google tag loading", () => {
  ...
  it("does not contact Google at all until advertising consent is granted", async () => {
    ...
    // …but consent state IS declared, defaulting to denied across all four v2
  ...
  it("sends the consent update before injecting the tag", async () => {
    const { consent, googleAds } = await loadModules();
    googleAds.initGoogleAds();
    consent.acceptAll();
...
describe("conversion taxonomy", () => {
  ...
  it("fires sign-up per creation event, not per browser, and never on a repeat", async () => {
    ...
    // Carries no payload at all — nothing about the account is transmitted.
...
describe("architecture guarantees", () => {
  ...
  it("renders nothing during prerender, with no window present at all", async () => {
    ...
    // installed would pass even if the component read storage during render —
